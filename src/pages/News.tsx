      ...
      <div className="flex flex-col md:flex-row justify-between items-end gap-8 mb-16">
        <div className="space-y-4">
          ...
          <h1 className="text-5xl md:text-7xl font-display font-medium text-white tracking-tighter">
            FideTV <span className="text-gray-600">Studio News.</span>
          </h1>
        </div>
        <p className="max-w-md text-gray-400 text-lg font-light leading-relaxed">
          Stay updated with the latest in media production, broadcasting technology, and community highlights.
        </p>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {news.map((item, idx) => (
            <motion.div
              ...
              viewport={{ once: true }}
              className="group glass rounded-[2.5rem] overflow-hidden border-white/5 hover:bg-white/5 transition-all duration-500"
            >
              <Link to={`/news/${item.slug}`} className="block h-full">
                <div className="relative aspect-[16/10] overflow-hidden bg-surface">
                  ...
                  <div className="absolute top-6 left-6 flex space-x-2">
                    <span className="bg-primary/20 backdrop-blur-md text-primary text-[10px] font-black uppercase tracking-wider px-3 py-1.5 rounded-full border border-primary/20">
                      Editorial
                    </span>
                  </div>
                </div>
                <div className="p-8 space-y-4">
                  ...
                  <div className="pt-4 flex items-center justify-between">
                    <div className="flex items-center space-x-2">
                      <div className="w-6 h-6 rounded-full bg-surface-bright flex items-center justify-center text-[10px] font-bold text-gray-400">
                        {item.profiles?.username?.[0] || 'A'}
                      </div>
                      <span className="text-xs text-gray-400">{item.profiles?.username || 'Admin'}</span>
                    </div>
