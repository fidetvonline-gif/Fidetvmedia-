  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      <div className="max-w-[1920px] mx-auto lg:h-[calc(100vh-80px)] flex flex-col lg:flex-row shadow-2xl">
        ...
        {/* Main Watch Area */}
        <div className="flex-grow flex flex-col relative z-10 border-r border-border-custom overflow-hidden">
          ...
          <div className="absolute top-6 left-6 z-20 flex items-center space-x-3 pointer-events-none drop-shadow-2xl">
            <div className={cn(
              ...
              (isPlayingFideTv && isFideTvLive) || !isPlayingFideTv 
                ...
                : "bg-background/60 text-foreground border-border-custom"
            )}>
            ...
            {isPlayingFideTv && isFideTvLive && (
              <div className="hidden sm:flex px-4 py-2 bg-background/60 backdrop-blur-xl rounded-full text-[10px] font-black text-foreground uppercase tracking-[0.3em] border border-border-custom items-center shadow-lg transition-all">
          ...
          <div className="relative w-full aspect-video lg:aspect-auto flex-grow bg-black group/player">
            {playerError ? (
              <div className="absolute inset-0 flex flex-col items-center justify-center bg-surface z-30">
                ...
                <h3 className="text-xl font-bold mb-2 text-foreground">Stream Unavailable</h3>
                <p className="text-foreground/60 text-sm mb-6 px-8 text-center italic">
                ...
                <div className="flex gap-4">
                  <button 
                    ...
                    className="px-6 py-2 bg-foreground/10 hover:bg-foreground/20 rounded-full text-xs font-bold uppercase tracking-widest border border-border-custom transition-all"
                  >
                    Retry Loading
                  </button>
                  <a 
              ...
              activeChannel.url?.includes('<iframe') ? (
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center bg-background overflow-hidden">
                  ...
                  <div className="relative z-10 flex flex-col items-center justify-center text-center p-8 space-y-6">
                    <div className="w-24 h-24 rounded-full bg-foreground/5 border border-border-custom flex items-center justify-center backdrop-blur-lg">
                      <Tv className="w-10 h-10 text-foreground/40" />
                    </div>
                    <div>
                      <h2 className="text-4xl font-display font-black tracking-tighter uppercase mb-2 text-foreground">Broadcast Offline</h2>
                      <p className="text-foreground/40 max-w-md mx-auto text-sm italic">
            ...
            <div className="absolute top-6 right-6 flex space-x-3 z-20 pointer-events-auto">
                <button 
                onClick={handleShare}
                className="p-3 bg-background/40 hover:bg-primary backdrop-blur-xl rounded-full text-foreground border border-border-custom transition-all opacity-0 group-hover/player:opacity-100"
                title="Share Channel"
                >
          ...
          <div className="h-24 lg:h-32 bg-surface border-t border-border-custom flex items-center px-6 lg:px-10 shrink-0 relative overflow-hidden">
             {/* gradient flare */}
             ...
             <div className="flex items-center justify-between w-full relative z-10 gap-x-6">
                 <div className="flex items-center gap-x-6">
                    <div className="w-14 h-14 rounded-2xl bg-foreground/5 border border-border-custom flex items-center justify-center shrink-0 shadow-lg">
                       {activeChannel.icon ? <activeChannel.icon className="w-6 h-6 text-foreground/40" /> : <Tv className="w-6 h-6 text-primary" />}
                    </div>
                    <div>
                       ...
                       <h2 className="text-xl lg:text-3xl font-display font-bold tracking-tight line-clamp-1 text-foreground">
                 ...
                 {isPlayingFideTv && isFideTvLive && ytStats && (
                    <div className="hidden sm:flex items-center space-x-8 px-6 py-3 bg-foreground/5 rounded-2xl border border-border-custom">
                        <div className="flex items-center space-x-3">
                           <Users className="w-5 h-5 text-foreground/40" />
                           <div className="flex flex-col">
                              <span className="text-[10px] text-foreground/40 font-bold uppercase tracking-widest">Watching Now</span>
                              <span className="text-lg font-bold text-foreground">{ytStats.viewers}</span>
                           </div>
                        </div>
                    </div>
                 )}
        ...
        <div className="w-full lg:w-[450px] xl:w-[500px] flex-shrink-0 bg-background flex flex-col h-[500px] lg:h-full z-20 shadow-[-20px_0_40px_rgba(0,0,0,0.1)]">
          {/* Tabs */}
          <div className="flex w-full border-b border-border-custom shrink-0">
             <button 
                onClick={() => setActiveTab('channels')}
                className={cn(
                  "flex-1 py-5 text-[11px] font-bold uppercase tracking-[0.2em] transition-all relative",
                  activeTab === 'channels' ? "text-foreground" : "text-foreground/40 hover:text-foreground"
                )}
             >
                Channel Guide
                {activeTab === 'channels' && (
             ...
             <button 
                ...
                className={cn(
                  ...
                  activeTab === 'chat' ? "text-foreground" : "text-foreground/40 hover:text-foreground"
                )}
             >
                <span>Live Chat</span>
          ...
          <div className="flex-grow overflow-hidden relative">
            ...
            <div className={cn(
              ...
              <div className="p-4 space-y-4">
                 <h3 className="text-[10px] font-bold uppercase tracking-widest text-foreground/40 px-2 pt-2">Your Broadcast</h3>
                 ...
                 <button
                    ...
                    className={cn(
                      ...
                      isPlayingFideTv 
                        ? "bg-foreground/10 border-border-custom shadow-xl" 
                 ...
                 >
                    ...
                    <div className="flex flex-col justify-center overflow-hidden">
                       ...
                       <h4 className="text-sm font-bold text-foreground truncate w-full">{customBroadcast.name}</h4>
                       <p className="text-xs text-foreground/40 truncate w-full italic">{isPlayingFideTv && event?.title ? event.title : 'Official Stream'}</p>
                    </div>
                 </button>

                 <div className="h-px w-full bg-border-custom my-4" />
                 <h3 className="text-[10px] font-bold uppercase tracking-widest text-foreground/40 px-2 pt-2">Live TV Networks</h3>
                       ...
                       >
                          ...
                          <div className="flex flex-col justify-center overflow-hidden">
                             <div className="flex items-center space-x-1.5 mb-0.5">
                                <channel.icon className="w-3 h-3 text-foreground/40" />
                                <span className="text-[10px] text-foreground/40 font-bold uppercase tracking-widest">{channel.category}</span>
                             </div>
                             <h4 className="text-sm font-bold text-foreground truncate w-full">{channel.name}</h4>
                             <p className="text-xs text-foreground/40 truncate w-full italic">{channel.description}</p>
                          </div>
                       </button>
                     );
                   })}
            ...
            {/* Chat View */}
            <div className={cn(
               "absolute inset-0 bg-surface flex flex-col transition-all duration-300",
               activeTab === 'chat' ? "translate-x-0 opacity-100" : "translate-x-full opacity-0 pointer-events-none"
            )}>
              {!event ? (
                 <div className="p-10 text-center flex flex-col items-center justify-center h-full">
                    <MessageSquare className="w-10 h-10 text-foreground/20 mb-4" />
                    <p className="text-foreground/40 text-sm italic">Live chat will be available when FideTV goes live.</p>
                 </div>
              ) : (
                <>
                  <div className="px-6 py-4 border-b border-border-custom bg-background shrink-0">
                     <p className="text-[10px] uppercase font-bold text-foreground/40 tracking-widest text-center">
                       Chatting in: <span className="text-foreground">{event.title}</span>
                     </p>
                  </div>
                  <div className="flex-grow overflow-hidden relative">
